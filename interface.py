...
# ════════════════════════════════════════════
#  ROI EXTRACTION
#  1. Gradient-based body zone: find the horizontal band with lowest
#     Sobel gradient (= uniform paint surface, not windows or wheels).
#  2. Specular removal: mask out blown highlights and deep shadows.
# ════════════════════════════════════════════
...
def specular_mask(hsv_img):
    """
    Keep only 'real body' pixels.
    Reject: specular highlights (low-sat + high-val) and shadows (very dark).
    Special case: if < 10% pixels survive, relax thresholds (white car).
    """
